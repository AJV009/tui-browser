...
const SettingsPanel = (() => {
  ...
  function renderEntries() {
    ...
    list.innerHTML = editingServers.map((s, i) => `
      <div class="settings-server-entry" data-index="${i}">
        ...
        <input type="text" data-field="tunnel" value="${esc(s.tunnel || '')}" placeholder="https://example.trycloudflare.com">
        <label>Local IPs (comma-separated)</label>
        <input type="text" data-field="local" value="${esc((s.local || []).join(', '))}" placeholder="192.168.1.10:7484">
    ...
    list.querySelectorAll('input').forEach(input => {
      input.addEventListener('input', (e) => {
        ...
        if (field === 'local') {
          editingServers[idx].local = e.target.value.split(',').map(s => s.trim()).filter(Boolean);
        } else {
  ...
  function addEntry() {
    editingServers.push({ name: '', tunnel: '', local: [] });
    renderEntries();
  ...
  async function save() {
    ...
    try {
      const res = await fetch('/api/servers', {
        ...
        body: JSON.stringify({ servers: valid }),
      });
      if (!res.ok) {
